   ...
   var  searchform = new Ext.FormPanel( { 
		...
		{  
		 ...
		 buttons : [ {
					...
					iconCls : "btn_query",
					handler : function() {   
					      var sql ="select date_format(create_time,'%Y-%m') as month,count(*) as total from order_info where 1=1 ";
					          if($("#beginDate").val()!='')
 								   sql += " and date_format(create_time,'%Y-%m') >='"+$("#beginDate").val()+"'";
					           if($("#endDate").val()!='')
					        	   sql += " and date_format(create_time,'%Y-%m') <='"+$("#endDate").val()+"'";
						      sql +=" group by date_format(create_time,'%Y-%m')";
					      var title="签单日统计报表"
					      var headers="签单月份,签单总数";
					      var alias="month,total";
					      var paramMap = {

...
var removeData = function(b) { 
	Ext.Msg.confirm("信息确认", "您确认要删除该记录吗？", function(c) {
		if (c == "yes") {
			Ext.Ajax.request( {
				url : "Busi_OrderCourseScore_delete.action",
				params : {
					id : b
				},
				method : "post",
...
function loadGirdStore(){ 
	...
	store.setBaseParam('_QUERY_n_eq_course_type',Ext.getCmp('QUERY_courseType')
			.getValue()) ;   
	store.setBaseParam('_QUERY_d_ge_exam_time',Ext.getCmp('QUERY_exam_startTime')
			.getValue()); 
	store.setBaseParam('_QUERY_d_le_exam_time',Ext.getCmp('QUERY_exam_endTime')
			.getValue());  
...
var formpanel =  new Ext.FormPanel( {
						...
						defaults : {
							...
							items : [
							      ...
							      ,{
										...
										items : [ {
											id : "QUERY_exam_startTime",
											xtype : "datefield",
											vtype : 'daterange',
											endDateField : 'QUERY_exam_endTime'
										}, {
											id : "QUERY_exam_endTime",
											xtype : "datefield",
											vtype : 'daterange',
											startDateField : 'QUERY_exam_startTime'
										} ]
									}
							      ]
						}]
	...
	g.getSelectionModel().each(function(e) {  
		new EditWindow(e.data.id,e.data.orderId); 
	});
	});  

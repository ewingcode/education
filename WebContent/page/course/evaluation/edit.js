var isUpload=0;
var orderId='';
var EditWindow = function(primaryId,orderAttachId,_orderId) { 
		...
		reader : new Ext.data.JsonReader( {
			  successProperty : 'success',
				root : 'result'
					...
					"orderId", "courseType", "chargerId","studentId","content",
					"operator", "orderAttachId",
			 ...
			 new Ext.form.ComboBox(
						{
						    ...
						    listeners:{ 
							   "select": function(c,r,i){
									...
									isUpload = paramValue; 
									 if(paramValue==1){ 
											 Ext.getCmp("uploadfile19").show(); 
											 // Ext.getCmp("uploadfile19").setDisabled(false);
											 
									 }else{ 
										 
											 $("input[name^='uploadfile19']").each(function(){
													var attach = $(this);
													attach.after(
															attach.clone(true).val(""));  
													attach.remove();
												} );
											  Ext.getCmp("uploadfile19").setDisabled(true);
											// Ext.getCmp("uploadfile19").hide(); 
		...
		editForm.getForm().load(
				{
					...
					url :  "Busi_OrderCourseEvaluation_query.action?condition=id="+this.primaryId,
					waitMsg : "正在载入数据...",

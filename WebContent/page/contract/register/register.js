...
var registerForm = new Ext.FormPanel( {
	...
	items : [ {
		...
		items : [ {
			...
			items : [ new Student.choose(false, "studentId",
			"studentName") ]
		} ]
	}, {
		xtype : 'fieldset',
	...
	}, new Order.fileUploadComp('初期辅导方案', "1", null, true, false),
			new Order.fileUploadComp('华实模拟协议', "2", null, true, false),
			new Order.fileUploadComp('方位测评报告', "3", null, true, false),
			new Order.fileUploadComp('教学安排表', "4", null, true, false) ],
	buttons : [ {
		text : "保存",
		iconCls : "btn_save",
		handler : function() {

...
<script><!--
		...
		var editform = new Ext.FormPanel( {
			...
			buttons : [ {
				...
				if (!Order.validateAttach(SUBMITTYPE_UPLOAD_FILE))
					return;
				lastSumitForm(editform, orderId,isOnlyEdit);
			}
			}, {
				text : "关闭",
				iconCls : "btn_cancel",
	 ...
	 * @return
	 */
	function lastSumitForm(editform, parentOrderId,isOnlyEdit) { 
		...
		Ext.Msg.confirm("信息确认", "您确认要保存该记录吗？", function(c) {
			if (c == "yes") {
				...
				editform.getForm().submit( {
					url : "Busi_OrderControl_lastOrder.action",
					method : "post",
					params : { 
					...
					success : function(i, j) {
						Ext.Msg.show( {
							...
							msg : '保存成功',
							fn : function() {
								parent.window.closeTabPanel(parentOrderId);
							},
							buttons : Ext.MessageBox.OK,
							icon : Ext.Msg.INFO
						});
